# -*- coding: utf-8 -*-
"""
이 스크립트는 논문에서 비교된 다양한 시계열 이상 탐지 모델의 PyTorch 구현을 포함합니다.

각 클래스는 특정 이상 탐지 모델의 아키텍처를 정의합니다. 포함된 모델은 다음과 같습니다:
- LSTM_Univariate: 각 변수에 대해 독립적인 LSTM을 사용하는 모델.
- Attention: 간단한 Multi-Head Self-Attention 모델.
- LSTM_AD: LSTM 기반 오토인코더.
- DAGMM: Deep Autoencoding Gaussian Mixture Model (ICLR 18).
- OmniAnomaly: VAE 기반 모델 (KDD 19).
- USAD: 이중 디코더를 사용한 적대적 학습 기반 오토인코더 (KDD 20).
- MSCRED: ConvLSTM 기반 오토인코더 (AAAI 19).
- CAE_M: 컨볼루션 오토인코더 (TKDE 21).
- MTAD_GAT: Graph Attention Network 기반 모델 (ICDM 20).
- GDN: Graph Deviation Network (AAAI 21).
- MAD_GAN: GAN 기반 모델 (ICANN 19).
- TranAD (및 변형): 논문에서 제안된 트랜스포머 기반 모델 (VLDB 22).
"""
import torch
...
## 간단한 Multi-Head Self-Attention 모델
...
# 제안 모델 (FCN) + Self Conditioning + Adversarial + MAML (VLDB 22)
...
# 제안 모델 + Self Conditioning + MAML (VLDB 22)
...
# 제안 모델 + Adversarial + MAML (VLDB 22)
...
# 제안 모델 + Self Conditioning + Adversarial + MAML (VLDB 22)
